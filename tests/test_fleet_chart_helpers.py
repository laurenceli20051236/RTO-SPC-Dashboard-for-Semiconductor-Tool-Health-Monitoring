...
def test_fleet_particle_chart_uses_threshold_logic_only() -> None:
    ...
    threshold_names = {str(trace.name) for trace in fig.data if "threshold" in str(trace.name)}
    assert threshold_names == {"High threshold", "Warning threshold"}
    assert not any("new text" in str(annotation.text).lower() for annotation in fig.layout.annotations)

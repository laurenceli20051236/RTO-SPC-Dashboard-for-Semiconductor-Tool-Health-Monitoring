    ...
    for _, group in ordered.groupby(group_columns, sort=False, dropna=False):
        ...
        for column, label, dash in limit_columns:
            if column in group and group[column].notna().any():
                ...
                limit_values = group[column].dropna().astype(float)
                fig.add_trace(
                    go.Scatter(
                        x=[1, 14],
                        y=[float(limit_values.iloc[0]), float(limit_values.iloc[-1])],
                        mode="lines",
    ...
    *,
    ...
    if note:
        fig.add_annotation(
            ...
            xref="paper",
            yref="paper",
            text=f"<i>Note: first 14 weekly PM points only.<br>{note_intro}</i><br>{note}",
            showarrow=False,
            align="center",
...
def _paper_particle_threshold_chart(df: pd.DataFrame, metric_name: str, title: str) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name="High threshold",
            showlegend=False,
            line=dict(color="#111111", dash="dot", width=1.4),
    ...
    fig.add_trace(
        go.Scatter(
            x=[0.5, 14.0],
            y=[thresholds["warning"], thresholds["warning"]],
            mode="lines",
            name="Warning threshold",
            showlegend=False,
            line=dict(color="#111111", dash="solid", width=1.4),
            hovertemplate="Warning=%{y:.4f}<extra></extra>",
        )
    )
